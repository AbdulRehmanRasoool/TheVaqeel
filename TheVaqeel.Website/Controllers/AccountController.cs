...
namespace TheVaqeel.Website.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {                 
            var navbarElements = context.HomeNavbars.SingleOrDefault(n => n.Id == 1);
            model.HomeNavbar = navbarElements;
            ...
            {
                var userEmail = UserManager.FindByEmail(model.LoginViewModel.Email);
                if(userEmail == null)
                {
                ...
                var lawyerStatus = context.Users.Where(lawyer => lawyer.Id == userId).Select(lawyer => lawyer.Status).FirstOrDefault();
                var userRole = UserManager.GetRoles(userId).FirstOrDefault();
                if (userRole.ToLower() == "lawyer")
                {
                    if (lawyerStatus == 0)
                    {
        ...
        public async Task<ActionResult> Register(HomeViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    model.RegisterViewModel.Role = "User";
                    //await SignInManager.SignInAsync(user, isPersistent:false, rememberBrowser:false);
                    await UserManager.AddToRoleAsync(user.Id, model.RegisterViewModel.Role);
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterLawyer(HomeViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.RegisterViewModel.Email, Email = model.RegisterViewModel.Email };
                ...
                if (result.Succeeded)
                {
                    model.RegisterViewModel.Role = "Lawyer";
                    //await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                    await UserManager.AddToRoleAsync(user.Id, model.RegisterViewModel.Role);
                    // For more information on how to enable account confirmation and password reset please visit https://go.microsoft.com/fwlink/?LinkID=320771
                    // Send an email with this link
